# HO13 Sample 1 — folder sorter

## Your task

Your Downloads folder is a graveyard of mystery files. Build a Cowork automation that sorts files into folders by type, date or keyword.

## What you will build

A data-processing automation inside Claude Cowork

## Files to upload

- A list of filenames (paste or upload a .txt file)

## How to run it

1. Open claude.ai → Cowork
2. Upload the files listed above
3. Paste the prompt from cowork-prompt.md (fill it in first!)
4. Claude will process your files and output the result

## Required

Claude.ai Pro or Team (Cowork access)
